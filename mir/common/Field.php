...
use mir\fieldTypes\Checkbox;
use mir\fieldTypes\Comments;
use mir\fieldTypes\Date;
use mir\fieldTypes\FieldParams;
use mir\fieldTypes\fieldParamsResult;
use mir\fieldTypes\File;
use mir\fieldTypes\ListRow;
use mir\fieldTypes\Number;
use mir\fieldTypes\Password;
use mir\fieldTypes\Select;
use mir\fieldTypes\StringF;
use mir\fieldTypes\TableNameField;
use mir\fieldTypes\Text;
use mir\fieldTypes\Tree;
use mir\fieldTypes\Unic;
use mir\fieldTypes\Chart;
...
class Field
{
        ...
        if (!empty($fieldData['linkTableName'])) {
            if (empty($fieldData['linkFieldError'])) {
                ...
                if ($this->table->getTableRow()['type'] === 'cycles') {
                    $tableWhere = $this->table->getMir()->getTableRow($fieldData['linkTableName']);
    ...
    public static function init($fieldData, aTable $table)
    {
        ...
        return $table->getMir()->fieldObjectsCaches(
            $staticName,
        ...
        switch ($action) {
            ...
            case 'modify':
                if ($editable = !empty($this->data['apiEditable'])) {
                    if (!$this->table->getMir()->getUser()->isCreator() && !empty($this->data['xmlRoles'])) {
                        if (count(array_intersect(
                            $this->data['xmlRoles'],
                            $this->table->getUser()->getRoles()
                        )) === 0) {
