...
class MirJson extends MirJsonParcer
{
    ...
    public function setMirCalculate($func)
    {
        $this->mirCalculate = $func;
    }
    ...
    protected function processMirParam(string $char)
    {
        switch ($char) {
            case '"':
            case "'":
                break;
            case '[':
                $this->MirParamStarts[$char]++;
                break;
            case ']':
                if ($this->MirParamStarts["["]) {
                    $this->MirParamStarts["["]--;
                } else {
                    $this->endMir();
                    $this->consumeChar($char);
    ...
    protected function endMir()
    {
        $this->listener->value(($this->mirCalculate)($this->buffer));
        $this->state = self::STATE_AFTER_VALUE;
        $this->buffer = '';
    }
}
