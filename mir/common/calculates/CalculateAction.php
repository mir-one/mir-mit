...
class CalculateAction extends Calculate
{
    ...
    protected function funcSchemaGzStringGet($params)
    {
        ...
        $pathDbPsql = $this->Table->getMir()->getConfig()->getSshPostgreConnect('pg_dump');

        $tmpFilename = tempnam(
            $this->Table->getMir()->getConfig()->getTmpDir(),
            $this->Table->getMir()->getConfig()->getSchema() . '.' . $this->Table->getUser()->getId() . '.'
        );

        $schema = $this->Table->getMir()->getConfig()->getSchema();
    ...
    protected function funcSchemaGzStringUpload($params)
    {
        ...
        $pathDbPsql = $this->Table->getMir()->getConfig()->getSshPostgreConnect('psql');


        $tmpFileName = tempnam(
            $this->Table->getMir()->getConfig()->getTmpDir(),
            $this->Table->getMir()->getConfig()->getSchema() . '.' . $this->Table->getMir()->getUser()->getId() . '.'
        );
        ...
        file_put_contents(
            ...
            sprintf(
                ...
                $this->Table->getMir()->getConfig()->getSchema(),
                preg_replace(
                    '/^(REVOKE|GRANT) ALL .*?;[\n\r]*$/m',
                    '',
        ...
        $tmpErrors = tempnam($this->Table->getMir()->getConfig()->getTmpDir(), 'schema_errors_');
    ...
    protected function funcReCalculateCycle($params)
    {
        if ($params = $this->getParamsArray($params)) {
            ...
            foreach ($Cycles as $cycleId) {
                ...
                $Cycle = $this->Table->getMir()->getCycle($params['cycle'], $tableRow['id']);
                $Cycle->recalculate();
            }
        }
    }
    ...
    protected function funcSchemaUpdate($params)
    {
        ...
        $MirInstall = new MirInstall(
            $this->Table->getMir()->getConfig(),
            $this->Table->getMir()->getUser(),
            null,
            $this->Table->getCalculateLog()
        );

        $params['schema'] = MirInstall::applyMatches($params['schema'], $params);
        if (empty($params['matches_name'])) {
            throw new errorException('Не определен источник схемы');
        }

        $MirInstall->updateSchema($params['schema'], true, $params['matches_name']);
    }
    ...
    protected function funcLinkToButtons($params)
    {
        ...
        $model = $this->Table->getMir()->getModel('_tmp_tables', true);
        ...
        $this->Table->getMir()->addToInterfaceDatas('buttons', $params);
    }
    ...
    protected function funcEmailSend($params)
    {
        ...
        try {
            $r = $this->Table->getMir()->getConfig()->sendMail(
                $params['to'],
                $params['title'],
                $params['body'],
                $params['files'] ?? [],
            ...
            if ($toBfl) {
                $this->Table->getMir()->getOutersLogger()->debug("email", $params);
            }
            return $r;
        } catch (Exception $e) {
            if ($toBfl) {
    ...
    protected function funcGetFromSoap($params)
    {
        ...
        try {
            ...
            if ($toBfl) {
                $this->Table->getMir()->getOutersLogger()->debug(
                    "SOAP",
    ...
    protected function funcLinkToFileDownload($params)
    {
        ...
        unset($file);
        $this->Table->getMir()->addToInterfaceDatas('files', ['files' => $files]);
    }
    ...
    protected function funcLinkToPanel($params)
    {
        ...
        if ($tableRow['type'] === 'calcs') {
            if ($topTableRow = $this->Table->getMir()->getTableRow($tableRow['tree_node_id'])) {
        ...
        if (!empty($params['id'])) {
            $ids = (array)$params['id'];
            foreach ($ids as $id) {
                $this->Table->getMir()->addLinkPanel(
                    $link,
                    $id,
                    [],
                    $params['refresh'] ?? false
    ...
    protected function funcLinkToPrint($params)
    {
        ...
        if (!$params['template'] || !($templates = $this->Table->getMir()->getModel('print_templates')->getAllIndexedByField(
            [],
            'styles, html, name',
            'name'
        )) || (!array_key_exists(
    ...
    protected function funcLinkToAnonymTable($params)
    {
        ...
        if ($d) {
            $t = $tableRow['id'] . '?d=' . urlencode(Crypt::getCrypted(
                ...
                $this->Table->getMir()->getConfig()->getCryptSolt()
            ));
        }
        return $this->Table->getMir()->getConfig()->getAnonymHost() . '/' . $this->Table->getMir()->getConfig()->getAnonymModul() . '/' . $t;
    }
    ...
    protected function funcLinkToTable($params)
    {
        ...
        if ($tableRow['type'] === 'calcs') {
            if ($topTableRow = $this->Table->getMir()->getTableRow($tableRow['tree_node_id'])) {
                ...
                $link .= $topTableRow['top'] . '/' . $topTableRow['id'] . '/' . $Cycle_id . '/' . $tableRow['id'];
                $Cycle = $this->Table->getMir()->getCycle($Cycle_id, $tableRow['tree_node_id']);
                $linkedTable = $Cycle->getTable($tableRow);
            } else {
                throw new errorException('Таблица циклов указана неверно');
            }
        } else {
            $linkedTable = $this->Table->getMir()->getTable($tableRow);
        ...
        $this->Table->getMir()->addToInterfaceLink(
            $link,
            $params['target'] ?? 'self',
    ...
    protected function funcTableLog($params)
    {
        if ($params = $this->getParamsArray($params)) {
            ...
            $this->Table->getMir()->mirActionsLogger()->innerLog(
        ...
        $this->__doAction(
            ...
            function ($params) {
                if (!empty($params['hash']) && strpos($params['hash'], 'i-') === 0) {
                    $hashData = TmpTables::init($this->Table->getMir()->getConfig())->getByHash(
