...
use mir\moduls\Forms\FormsController;
...
class CalculcateFormat extends Calculate
{
    ...
    public function getPanelFormat($fieldName, $row, $tbl, aTable $table, $Vars = [])
    {
        ...
        if ($buttons) {
            ...
            do {
                ...
                $key = ['table_name' => '_panelbuttons', 'user_id' => $this->Table->getMir()->getUser()->getId(), 'hash' => $hash];
    ...
    {
        if ($this->Table->getMir()->getSpecialInterface() !== 'form') {
            return [];
        }
