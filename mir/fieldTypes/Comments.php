...
class Comments extends Field
{
    ...
    public function addViewValues($viewType, array &$valArray, $row, $tbl = [])
    {
        ...
        switch ($viewType) {
            ...
            case 'web':
                $n = count($valArray['v']);
                $isCuted = false;
                if ($n > 0 && ($valArray['v'][$n - 1][1] !== $this->table->getMir()->getUser()->getId())) {
    ...
    protected function checkValByType(&$val, $row, $isCheck = false)
    {
        ...
        if (is_string($val)) {
            $val = [[date('Y-m-d H:i'), $this->table->getMir()->getUser()->getId(), $val]];
        } elseif (is_array($val)) {
            foreach ($val as $comment) {
                if (!preg_match(
    ...
    protected function modifyValue($modifyVal, $oldVal, $isCheck, $row)
    {
        ...
        if (is_string($modifyVal)) {
            if ($modifyVal !== '') {
                $oldVal[] = [date('Y-m-d H:i'), $this->table->getMir()->getUser()->getId(), $modifyVal];
            }
    ...
    {
        return $this->table->getMir()->getModel(static::table_viewed, true);
    }
    ...
    private function prepareComment($commentArray, $textCut, &$isCuted)
    {
        ...
        $commentArray[1] = $this->table->getMir()->getNamedModel(UserV::class)->getFio($commentArray[1]);
        return $commentArray;
    }
    ...
    {
        return $this->table->getMir()->getConfig()->getSql()->getField('select nums from ' . static::table_viewed
    ...
    public function setViewed(int $nums, $row_id)
    {
        ...
        if (!empty($this->data['linkTableName']) && !empty($this->data['linkFieldName'])) {
            $LinkedTable = $this->table->getMir()->getTableRow($this->data['linkTableName']);
            $vars = [$nums,
                $LinkedTable['id'],
