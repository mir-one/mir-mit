...
class File extends Field
{
    ...
    protected function modifyValue($modifyVal, $oldVal, $isCheck, $row)
    {
        ...
        if (!$isCheck) {
            ...
            if ($deletedFiles) {
                static::deleteFilesOnCommit($deletedFiles, $this->table->getMir()->getConfig());
            }
        }
        return $modifyVal;
    }
        ...
        if (!$isCheck && ($this->data['category'] !== 'column' || $row['id'] ?? null)) {
            ...
            foreach ($val as $file) {
                ...
                if (!empty($file['tmpfile'])) {
                    if (!is_file($ftmpname = $this->table->getMir()->getConfig()->getTmpDir() . $file['tmpfile'])) {
                    ...
                    $this->table->getMir()->getConfig()->getSql()->addOnCommit(function () use ($ftmpname, $fname) {
                    ...
                    $filepath = static::getFilePath($file['file'], $this->table->getMir()->getConfig());
                    $fl['file'] = $file['file'];
                    ...
                    if (key_exists($filepath, static::$transactionCommits)) ; elseif (!is_file($filepath)) {
                        ...
                        if (strpos($file['file'], $fPrefix) !== 0 && !empty($this->data['fileDuplicateOnCopy'])) {
                            ...
                            $otherfname = static::getFilePath($file['file'], $this->table->getMir()->getConfig());

                            static::$transactionCommits[$fname] = $otherfname;

                            $this->table->getMir()->getConfig()->getSql()->addOnCommit(function () use ($otherfname, $fname) {
