...
class User extends Model
{
    ...
    {
        if (!is_null($userRow) || $userRow = $this->Mir->getNamedModel(UserV::class)->getById($id)) {
    ...
    public function update($params, $where, $oldRow = null): int
    {
        ...
        if (array_key_exists('boss_id', $decoded)) {
            ...
            if (key_exists('boss_id', $oldRow)) {
                ...
                $oldBoss = $this->Mir->getNamedModel(UserV::class)->getField('boss_id', ['id' => $where['id']]);
            }
        }
...
select * from subUsers WHERE ' . $this->getWhere(['id' => $bossId]))
    ...
    public function getAllConnectedUsers($id, $userRow = null, &$checkedUsers = [])
    {
        ...
        $checkedUsers[] = (int)$id;
        if (is_null($userRow)) {
            $userRow = $this->Mir->getNamedModel(UserV::class)->getById($id);
        }
        ...
        $connectedUsers = array_merge(
            $connectedUsers,
            $this->Mir->getNamedModel(UserV::class)->getField('id', ['boss_id' => $id], null, null)
        );
