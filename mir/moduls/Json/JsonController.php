...
class JsonController extends Controller
{
    ...
    {
        $this->modulePath = $this->mirPrefix . '/Json/';
        ...
        try {
            ...
            $this->authUser();
            $this->Mir = new Mir($this->Config, $this->aUser);
            if ($this->aUser->isCreator() && $this->arrayIn['withLogs'] ?? null) {
                $this->Mir->setCalcsTypesLog(is_array($this->arrayIn['withLogs']) ? $this->arrayIn['withLogs'] : ['c', 'a']);
            }
            ...
            foreach ($this->Mir->getInterfaceLinks() ?? [] as $link) {
    ...
    protected function jsonRemotes()
    {
        $selectedRemotes = [];
        $remoteOutputs = [];
        $RemotesTable = $this->Mir->getTable('ttm__remotes');
    ...
    protected function checkTable($isRequestForWrite)
    {
        try {
            if ($this->inModuleUri === '') {
                ...
                if (!($tableRow = $this->Mir->getTableRow($cycleTableId))) {
                    throw new errorException('');
                }

                $Cycle = $this->Mir->getCycle($cycleId, $cyclesTableId);
                $this->Table = $Cycle->getTable($tableRow);
            } elseif (preg_match('/^(\d+)$/', $this->inModuleUri, $match)) {
                $tableId = $match[1];

                if (!($tableRow = $this->Mir->getTableRow($tableId))) {
                    throw new errorException('');
                }

                $this->Table = $this->Mir->getTable($tableRow);
            } else {
                throw new errorException('');
            }
    ...
    protected function sendJson($error, $errorDescription)
    {
        ...
        if ($this->Mir && $this->aUser->isCreator() && $this->arrayIn['withLogs'] ?? false) {
            $this->arrayOut['logs'] = $this->Mir->getCalculateLog()->getLodTree();
        }
