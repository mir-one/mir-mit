...
class AdminTableActions extends WriteTableActions
{
    ...
    {
        if (!$this->Mir->getTableRow('ttm__charts')) {
            throw new errorException('Таблица графиков не надена');
        }
        $result['chartTypes'] = [];
        foreach ($this->Mir->getModel('ttm__charts')->executePrepared(
            true,
            [],
    ...
    {
        $tables = [];
        $fields = TablesFields::init($this->Mir->getConfig())->getAll(
            ['is_del' => false],
            'name, table_id, title, data, category'
        );

        foreach (Table::init($this->Mir->getConfig())->getAll(
            ['is_del' => false],
            'name, id, title'
        ) as $tRow) {
            $tFields = [];
    ...
    public function refresh_cycles()
    {
        ...
        $tables = [];
        foreach ($ids as $id) {
            $Cycle = $this->Mir->getCycle($id, $this->Table->getTableRow()['id']);

            if (empty($tables)) {
                $tables = $Cycle->getTableIds();
                foreach ($tables as &$t) {
                    $t = $this->Mir->getTableRow($t);
                }
                unset($t);
            }
