...
use mir\common\FormatParamsForSelectFromTable;
use mir\common\Mir;
...
class ReadTableActions extends Actions
{
    ...
    public function setTableFavorite()
    {
        if ($this->post['status']) {
            ...
            if (key_exists(
                ...
                $Users = $this->Table->getMir()->getTable('users');
                if ($status) {
    ...
    public function refresh()
    {
        ...
        if ($this->post['recalculate'] ?? false) {
            try {
                ...
                $error = $e->getMessage();
                if ($this->Mir->getUser()->isCreator()) {
                    $error .= ' <br/> ' . $e->getPathMess();
                    $result['error'] = $error;
                }
                $this->Mir->transactionRollback();
    ...
    {
        $template = $this->Mir->getModel('print_templates')->executePrepared(
            true,
            ['name' => 'main'],
            'styles, html'
        )->fetch();
...
table tr td.title{font-weight: bold}', 'html' => '{table}'];
    ...
    public function checkTableIsChanged()
    {
        ...
        $Table = $this->Mir->getTable($table_id, $cycle_id, true);
        $i = 0;
        do {
            if ($i > 0) {
                sleep(3);
    ...
    protected function getTableClientForm(): array
    {
        ...
        if ($this->User->isCreator()) {
            $result['ROLESLIST'] = $this->Mir->getModel('roles')->getFieldIndexedById(
                'title',
                ['is_del' => false]
            );
        } else {
        ...
        $addLinkToSelectTableSinFields = function (&$fields) {
            foreach ($fields as $f) {
                ...
                if (!empty($f['selectTable'])) {
                    if ($table = $this->Mir->getTableRow($f['selectTable'])) {
                        ...
                        if ($table['type'] !== 'calcs') {
                            ...
                            $topTable = $this->Mir->getTableRow($table['tree_node_id']);
    ...
    public function click()
    {
        ...
        if ($click = is_string($this->post['data']) ? (json_decode(
            ...
            if ($click['item'] === 'params') {
                ...
                $row = TmpTables::init($this->Mir->getConfig())->getByHash(
                    '_insert_row',
                    $this->User,
                    $click['item']
                );
    ...
    public function getFieldLog()
    {
        ...
        $logs = $this->Mir->mirActionsLogger()->getLogs(
            $this->Table->getTableRow()['id'],
        ...
        if (empty($logs)) {
            ...
            $tmp = $this->Mir->getTable('log_structure');
            ...
            $this->Table->getMir()->addToInterfaceDatas('table', $table);
        }
    }
    ...
    protected function fieldsForClient($fields)
    {
        ...
        foreach ($fields as &$field) {
            ...
            foreach (Mir::FIELD_CODE_PARAMS as $param) {
            ...
            if (!$this->User->isCreator()) {
                foreach (Mir::FIELD_ROLES_PARAMS as $param) {
                    unset($field[$param]);
                }
                unset($field['logging']);
        ...
        if ($this->Table->getTableRow()['name'] === 'tables_fields') {
            if ($this->Mir->getUser()->isCreator()) {
                $fields['data_src']['jsonFields']['fieldSettings']['editRoles']['values']
                    ...
                    = $this->Mir->getModel('roles')->getFieldIndexedById(
                        'title',
                        ['is_del' => false],
                        'title->>\'v\''
                    );
                $fields['data_src']['jsonFields']['fieldSettings']['selectTable']['values'] = $this->Mir->getModel('tables')->getFieldIndexedByField(
                    ['is_del' => false],
                    'name',
                    'title',
                    'title->>\'v\''
            ...
            if (!empty($id)) {
                if ($this->Table->loadFilteredRows('web', [$id])) {
                    if (key_exists('button_to_cycle', $this->Table->getFields())) {
                        ...
                        if ($CalcsId = $this->Mir->getModel('tables')->getField(
                            'id',
    ...
    {
        $return = [];
        if ($force || $this->Table->getTableRow()['type'] === 'tmp' || $this->Mir->isAnyChages() || !empty($data['refresh']) || $this->Mir->getConfig()->procVar()) {
            $this->Table->reCalculateFilters(
                'web',
                false,
                false,
