...
use mir\common\controllers\WithAuthTrait;
...
class TableController extends interfaceController
{
    ...
    {
        $tmpTables = $this->Mir->getTable('roles')->getByParams(
            [
            'field' => 'tmp_for_favorites', 'where' => [
                ['field' => 'id', 'operator' => '=', 'value' => $this->Mir->getUser()->getRoles()],
        ...
        if ($tmp) {
            $tmp_tables=$this->Mir->getTable('tables')->getByParams(
                [
                'field'=>['id', 'title'],
                'where'=>[
    ...
    public function actionAjaxActions(ServerRequestInterface $request)
    {
        ...
        if ($this->User->isCreator() && $this->CalculateLog && ($types = $this->Mir->getCalculateLog()->getTypes())) {
    ...
    protected function setTreeData()
    {
        ...
        if ($this->Table && ($this->Table->getTableRow()['type'] === 'calcs') && $this->Cycle) {
            ...
            foreach ($this->Cycle->getViewListTables() as $i => $tName) {
                if ($tableRow = $this->Mir->getTableRow($tName)) {
                    $tId = $tableRow['id'];
    ...
    protected function outputHtmlTemplate()
    {
        try {
            ...
            $this->__addAnswerVar('schema_name', $this->Config->getSettings('mir_name'), true);
            ...
            $this->__addAnswerVar('mirFooter', $this->Config->getMirFooter());
    ...
    public function __actionRun($action, ServerRequestInterface $request)
    {
        ...
        $this->Mir->setCalcsTypesLog(json_decode($request->getCookieParams()['pcTableLogs'] ?? '[]', true));
    ...
    public function actionTable(ServerRequestInterface $request)
    {
        ...
        if ($this->User->isCreator()) {
            ...
            if (($types = $this->Mir->getCalculateLog()->getTypes())) {
                if (in_array('flds', $types)) {
    ...
    protected function checkTableByUri(ServerRequestInterface $request)
    {
        ...
        if ($tableUri && (preg_match(
            ...
            if (empty($tableMatches[3])) {
                ...
                $branchData = $this->Mir->getTable('tree')->getByParams(
            ...
            $this->Cycle = $this->Mir->getCycle($tableMatches[2], $tableMatches[1], $this->Mir);
            ...
            $branchData = $this->Mir->getModel('tree')->executePrepared(
                false,
                ['id' => $this->branchId],
            ...
            switch ($branchData['type']) {
                ...
                case 'anchor':
                    $this->anchorId = $this->branchId;
                    $this->branchId = $branchData['top'];
                    $this->Table = $this->Mir->getTable($branchData['default_table']);
