...
class WriteTableActions extends ReadTableActions
{
    ...
    public function add()
    {
        ...
        if ($this->Table->getTableRow()['name'] === 'tables_fields' && key_exists(
            ...
            $this->Mir->getModel('tables_fields')->setAfterField($this->post['tableData']['afterField']);
        }
    ...
    {
        return File::fileUpload($this->User->getId(), $this->Mir->getConfig());
    }
    ...
    public function checkInsertRow()
    {
        ...
        if (empty($this->post['hash'])) {
            ...
            } while (!TmpTables::init($this->Mir->getConfig())->saveByHash(
    ...
    public function csvImport()
    {
        if ($this->Table->isUserCanAction('csv_edit')) {
            ...
            if (is_array($r) && ($r['ok'] ?? false)) {
                $this->Mir->addToInterfaceLink($this->Request->getServerParams()['REQUEST_URI'], 'self', 'reload');
            }
            return $r;
        } else {
