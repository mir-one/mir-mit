...
abstract class RealTables extends aTable
{
    ...
    {
        if ($force) {
            return $this->Mir->getNamedModel(Table::class)->getField('updated', ['id' => $this->tableRow['id']]);
        }
        return $this->updated;
    }
    ...
    {
        $field = static::getFullField($this->Mir->getModel('tables_fields__v', true)->getById($fieldId));
        if ($field['category'] === 'column') {
            $this->Mir->getModel($this->tableRow['name'])->addColumn($field['name']);
            if ($field['type'] === 'checkbox') {
                $this->Mir->getModel($this->tableRow['name'])->update(
    ...
    {
        if ($columnName !== 'id') {
            $this->Mir->getModel($this->tableRow['name'])->createIndexOnJsonbField($columnName);
        }
    }
    ...
    {
        if ($columnName !== 'id') {
            $this->Mir->getModel($this->tableRow['name'])->removeIndex($columnName);
        }
    }
    ...
    public function deleteField($fieldRow)
    {
        if ($fieldRow['category'] === 'column') {
            $this->Mir->getModel($this->tableRow['name'])->dropColumn($fieldRow['name']);
        }
    }
        ...
        if ($this->tableRow['deleting'] === 'none' && !$isInnerChannel) {
            ...
            switch ($this->tableRow['deleting']) {
                ...
                case 'delete':
                    if ($this->tableRow['type'] === 'cycles') {
                        foreach ($remove as $id) {
                            $this->Mir->deleteCycle($id, $this->tableRow['id']);
                        }
                    }
            ...
            if (in_array($channel, ['web', 'xml']) || $this->recalculateWithALog) {
                foreach ((array)$remove as $id) {
                    $this->Mir->mirActionsLogger()->delete(
                        $this->tableRow['id'],
                        null,
                        $id,
    ...
    public function restoreRows($restore, $channel)
    {
        ...
        if (in_array($channel, ['web', 'xml']) || $this->recalculateWithALog) {
            foreach ((array)$restore as $id) {
                $this->Mir->mirActionsLogger()->restore($this->tableRow['id'], null, $id);
            }
        }
        /******aLog*****/
    ...
    public function saveTable()
    {
        ...
        if (!$this->Mir->getNamedModel(Table::class)->update($update, $where)) {
    ...
    protected function duplicateRow($channel, $baseRow, $replaces, $addAfter)
    {
        ...
        if ($row && $this->tableRow['name'] === 'tables') {
            $this->changeIds['rowOperations'][] = function () use ($baseRow, $row) {
                $this->Mir->getNamedModel(Table::class)->dulpicateTableFiedls($row, $baseRow);
            };
            $this->tbl['rows'] = [];
        }
